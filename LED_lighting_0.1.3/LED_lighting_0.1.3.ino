...
// Version 0.1.3
// 9/04/18
// added a new color loop that will focus more on the red and blue lights, and instead of the whole screen 
// being the same color I sectioned the display into 4 sub displays each with their own color/color loop
...
// this color controller doesnt have green
void ngColorController()
{
  if(red == low)
  {
    red = high;
...
// *****************************************************************
// as of now all the color loops are designed to be used on their own. No testing has been done 
// to see if they will work in tandem, but the guess based on how coding works and how this code
// is set up, they likely will not work together
// ******************************************************************

// color loop for the seedling stage, very similar to veg loop but there is no green
// red promotes growth, as does blue, but do not want to much blue as it stops growth if there is too much
void seedlingColorLoop()
{
  for(int i = 0; i < 64; i++)
  {
    ...
    if(i%3 == 0)
    {
      matrix.drawLine(0,i,64,i,matrix.Color333(red,0,high));
    }
    // make these the red lines
    else
    {
      matrix.drawLine(0,i,64,i,matrix.Color333(high,0,blue));
    }
  }
  ngColorController();
}

// similar to seedling loop but now includes some green/yellow lights
void vegColorLoop()
{
  for(int i = 0; i < 64; i++)
  {
    ...
    //throw in some green and red
    if(i%4 == 0)
    {
      matrix.drawLine(12,i,50,i,matrix.Color333(high,green,0));
    }
  }
  colorController();
}

// want more blue here to stop the plant from growing 
void flowerColorLoop()
{
  for(int i = 0; i < 64; i++)
  {
    // make these the red lines
    ...
    //throw in some green and blue
    if(i%4 == 0)
    {
      matrix.drawLine(12,i,50,i,matrix.Color333(0,green,high));
    }
  }
}
...
{  
  ...
  blue = 0;
  green = 0;
  period1 = 1000*60*60*2; // milliseconds*seconds*minutes*hours*days
